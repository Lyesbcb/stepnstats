...
if __name__ == "__main__":
    ...
    for index, item in enumerate(items):
        ...
        gitem = cv.cvtColor(item, cv.COLOR_BGRA2GRAY)
        ih, iw = item.shape[:2]
        matches = []
        ...
        if matches != []:
            ...
            json.update(
                {contentWithNb: result, quantityWithNb: resultQuantity})
    print(json)
